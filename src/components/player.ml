  ...
  e#resolve#set (fun _ t ->
    ...
    | Wall.HWall w ->
            ...
            if w#position#get.y < e#position#get.y then
                e#position#set {x = e#position#get.x; y = w#position#get.y +. float w#box#get.height}
            else
                e#position#set {x = e#position#get.x; y = w#position#get.y  -. float height};
            e#velocity#set Vector.zero
    | _ -> ());
...
let players () =
  ...
  , player
      Cst.("player2", paddle2_x, paddle2_y, paddle_color, paddle_width, paddle_height) )
;;
